...
class _YourWidgetState extends State<YourWidget> {
  ...
  void _refreshData() async {
    ...
    List<TodaysEnergy> updatedProductList = await generateProductList();
    setState(() {
      productList = updatedProductList;
    });
  }

  Future<List<TodaysEnergy>> generateProductList() async {
    var response =
    await http.get(Uri.parse('http://103.149.143.33:8081/api/TodayEnergy'));
    var decodedProducts =
    json.decode(response.body).cast<Map<String, dynamic>>();
    List<TodaysEnergy> productList = await decodedProducts
        .map<TodaysEnergy>((json) => TodaysEnergy.fromJson(json))
        .toList();
    return productList;
  }
    ...
    dataGridRows = productList.map<DataGridRow>((dataGridRow) {
      return DataGridRow(
        cells: [
          ...
          DataGridCell(
              columnName: 'TodaysGeneration',
              value: dataGridRow.todaysGeneration),
